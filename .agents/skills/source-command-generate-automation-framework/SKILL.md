---
name: "source-command-generate-automation-framework"
description: "Thiết kế và scaffold automation framework hoàn chỉnh. Hỗ trợ Playwright, Selenium, Appium — Web, Mobile, API."
---

# source-command-generate-automation-framework

Use this skill when the user asks to run the migrated source command `generate_automation_framework`.

## Command Template

# Workflow: Thiết Kế Automation Framework

> **BẮT BUỘC (MANDATORY SKILL):** Bạn PHẢI nạp và đọc kỹ nội dung của skill **`framework_architect`** (tại `.Codex/skills/framework_architect/SKILL.md`) trước khi bắt đầu. Ngoài ra, tham khảo thêm skill **`qa_automation_engineer`** để nắm các quy tắc automation chung.
