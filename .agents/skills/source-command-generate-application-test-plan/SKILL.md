---
name: "source-command-generate-application-test-plan"
...
# source-command-generate-application-test-plan

Use this skill when the user asks to run the migrated source command `generate_application_test_plan`.

## Command Template

# Workflow: Khám Phá Ứng Dụng & Sinh Test Plan

> **BẮT BUỘC (MANDATORY SKILL):** Bạn PHẢI nạp và đọc kỹ nội dung của skill **`qa_automation_engineer`** (tại `.Codex/skills/qa_automation_engineer/SKILL.md`) trước khi bắt đầu. Ngoài ra, tham khảo thêm skill **`requirements_analyzer`** và **`ui_debug_agent`** để hỗ trợ phân tích UI.
