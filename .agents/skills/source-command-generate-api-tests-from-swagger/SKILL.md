---
name: "source-command-generate-api-tests-from-swagger"
...
# source-command-generate-api-tests-from-swagger

Use this skill when the user asks to run the migrated source command `generate_api_tests_from_swagger`.

## Command Template

# Command: Sinh API Tests từ Swagger/OpenAPI

> **BẮT BUỘC (MANDATORY SKILL):** Bạn PHẢI nạp và đọc kỹ nội dung của skill **`qa_automation_engineer`** (tại `.Codex/skills/qa_automation_engineer/SKILL.md`) trước khi bắt đầu. Ngoài ra, tham khảo thêm skill **`test_data_generator`** để sinh test data đúng chuẩn.
