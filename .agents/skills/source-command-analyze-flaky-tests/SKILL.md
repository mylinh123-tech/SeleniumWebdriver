---
name: "source-command-analyze-flaky-tests"
...
# source-command-analyze-flaky-tests

Use this skill when the user asks to run the migrated source command `analyze_flaky_tests`.

## Command Template

# Workflow: Phân Tích & Khắc Phục Flaky Tests

> **BẮT BUỘC (MANDATORY SKILL):** Bạn PHẢI nạp và đọc kỹ nội dung của skill **`flaky_test_analyzer`** (tại `.Codex/skills/flaky_test_analyzer/SKILL.md`) trước khi bắt đầu. Ngoài ra, tham khảo thêm skill **`smart_locator_agent`** và **`locator_healer_agent`** khi cần sửa/thay locator.
...
1. **Sửa code** theo thứ tự ưu tiên (fix nghiêm trọng nhất trước):
   ...
   **Fix Locator:**
   ...
   - Tuân thủ locator priority trong `.Codex/rules/locator_strategy.md`
