...
public class ProjectsPage extends BasePage {
    ...
    public ProjectsPage selectCustomer(String customerId, String customerName) {
        wait.until(ExpectedConditions.presenceOfElementLocated(selectCustomer));
        String js =
        ...
        wait.until(driver -> driver.findElement(buttonCustomerDropdown).getAttribute("title").contains(customerName));
    ...
    public ProjectsPage fillDescription(String description) {
        wait.until(ExpectedConditions.presenceOfElementLocated(iframeDescription));
        String js =
    ...
    public ProjectsPage openEditProjectPageFromCurrentProject() {
        ...
        driver.get(ConfigData.Base_URL + addNewProjectPageUrl + "/" + projectId);
        ActionKeyword.waitForPageLoaded(driver);
        wait.until(ExpectedConditions.visibilityOfElementLocated(inputProjectName));
    ...
    public ProjectsPage openProjectProfilePage(String projectId) {
        driver.get(ConfigData.Base_URL + projectProfilePageUrl + projectId);
        ActionKeyword.waitForPageLoaded(driver);
        wait.until(ExpectedConditions.urlContains(projectProfilePageUrl + projectId));
    ...
    public ProjectsPage searchProject(String projectName) {
        ...
        driver.findElement(inputSearchProject).sendKeys(projectName);
    ...
    public ProjectsPage deleteProjectByName(String projectName) {
        ...
        WebElement projectElement = wait.until(ExpectedConditions.visibilityOfElementLocated(projectNameLink));
        new Actions(driver).moveToElement(projectElement).perform();
    ...
    public String getProjectNameValue() {
        return getInputValue(inputProjectName);
    }
    ...
    public String getStartDateValue() {
        return getInputValue(inputStartDate);
    }

    public String getDeadlineValue() {
        return getInputValue(inputDeadline);
    }

    public String getDescriptionValue() {
        wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(iframeDescription));
        wait.until(ExpectedConditions.visibilityOfElementLocated(textareaDescription));
        String description = driver.findElement(textareaDescription).getText();
    ...
    private void selectPickerBySearchText(By selectLocator, By dropdownButton, String visibleText) {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(inputSelectPickerSearch));
        driver.findElement(inputSelectPickerSearch).clear();
        String searchKeyword = visibleText.length() > 20 ? visibleText.substring(0, 20) : visibleText;
        driver.findElement(inputSelectPickerSearch).sendKeys(searchKeyword);
        ...
        wait.until(driver -> driver.findElement(dropdownButton).getAttribute("title").contains(visibleText));
    }
